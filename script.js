...
const getCard = () => {
    let randomIndex = CARDS[Math.floor(Math.random() * 52)]
   const img = document.createElement('img')
   img.src = randomIndex.image
   document.querySelector('#deck-of-cards').appendChild(img)
   let value = document.createElement('H3')
   value.textContent = `Points = ${randomIndex.value}`
...
const showAll = () => {
    ...
    if (input.value >= CARDS[i].value){
        const img2 = document.createElement('img')
        img2.src =CARDS[i].image
        document.querySelector('#deck-of-cards').appendChild(img2)
        let value2 = document.createElement('H3')
        value2.textContent = `Points = ${CARDS[i].value}`
        document.querySelector('.buttons').appendChild(value2)
        }
    }
...
// if input value is greater than CARDS.value do get caRDS